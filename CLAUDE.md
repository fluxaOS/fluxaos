...
- `project.target_repo_path` (DB-only, FLX-221) — absolute path to a local clone of each project's target repo on `main`, stored as a per-project column on the `project` table. Nullable; the stage-runner throws `MissingProjectTargetRepoPathError` when null at acquire time. Edited via Settings → Projects (FLX-207 makes the form field editable). Operators upgrading from the pre-FLX-221 env-backed value must copy it into the column via Settings or SQL — there is no auto-backfill.
