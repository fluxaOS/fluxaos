...
6. **R-REM-W3-a** — Anthropic port cleanup + live-Claude journey — **Scoped (2026-04-20), not planned.** Small cleanup + verification phase. Three items: delete the unused `src/core/ports/ai.ts` (zero consumers, SDK-shaped misfit — brainstorm exploration established the orchestrator's real AI path is `executeStageRun` → `SubprocessExecutor` → `claude` binary → `SubprocessStdoutParser`, already wired via seed data and R-REM-W2's parser adapter); update the R-AUDIT triage Pattern 2 entry with a resolution note; run a new Playwright journey `e2e/real-anthropic-stage-run.spec.ts` with live `ANTHROPIC_API_KEY` to prove the end-to-end engine works against real Claude. Estimated 1 session. Alpha-unlock milestone: converts "engine expected to work" to "engine observed to work." See [disposition-design](superpowers/specs/2026-04-20-r-ui-2-disposition-and-w3-decomposition-design.md) §"Phase 3 — R-REM-W3-a" for full scope.
