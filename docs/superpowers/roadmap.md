...
| R-REM-W1 — Foundation remediation (invariant 7, CRUD factory, dead-code purge) | **In flight — Tasks 1-6 shipped, 7-9 pending** | [wave-1-plan](superpowers/plans/2026-04-17-wave-1-foundation-plan.md) | — (triage supersedes spec: [triage](superpowers/audits/2026-04-17-audit-triage.md)) |
...
4. **R-REM-W1** — Foundation remediation — **In flight.** Tasks 1-6 shipped on `feat/wave-1-foundation` (invariant 7 amended, ARCHITECTURAL_STANDARDS retired, CRUD factory rewritten with versioned variant, dead exports + ~1,100 LOC of dead source deleted). Tasks 7-9 pending on a follow-up branch: drop 3 dead schema tables + remaining router procedures, relocate out-of-core files (+fix pre-existing events.ts tsc error), end-to-end verify with browser confirmation. See [Tasks 1-6 handoff](superpowers/handoffs/2026-04-18-wave-1-tasks-1-6-session-handoff.md).
