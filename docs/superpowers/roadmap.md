...
| Phase | Status | Scope |
|-------|--------|-------|
| **R-POLISH-DOCS — Cleanup, terminology, ship docs** | **Next** | Terminology passes. README updates so an operator can stand fluxaOS up from a fresh clone. Attribution to Archon in any spec that lifted patterns. Replaces the old R-REM-W4 and R6 placeholders. (Engine-correctness items already shipped in R-POLISH-CORE; drizzle meta rebaseline deferred to DEF-025.) |

**Dependency ordering:** R-DAEMON + R-SETTINGS-ALPHA + R-MISSION-CONTROL + R-SMOKE + R-POLISH-CORE done. R-POLISH-DOCS is last.
