...
Issue tracking lives in **Linear** (workspace `rebos`, team `FLX`). Active projects: **Alpha Release**, **Bug Backlog** (bug findings go here), **Enhancements**, **Settings & config integrity**, **Post-Alpha Roadmap**, and **Post-Alpha Wishlist**. The repo's `docs/superpowers/deferred-fixes.md` is frozen — historical record only.

**2026-06-10 status update:** the FLX-239 tenancy epic is **Done** — all 8 stages shipped (PRs #383–385, #400 `b41026e`, #401 `3b5c45e`, #402 `45b7380`). Tenancy is UUID-only (`/p/{uuid}` routes), slug columns are dropped (migration `0033_flx_271_drop_tenancy_slugs.sql`), session-user auth lives inside `resolveContext`, and the deterministic seed project UUID `00000000-0000-4000-8000-000000000001` is the `FLUXAOS_PROJECT_ID` producer. Full Playwright baseline: 91 passed / 0 failed / 4 skipped. Same-day bug fixes: FLX-264 / FLX-265 / FLX-270 (PRs #403 / #404 / #405 — issue-watcher now fails fast on missing dispatch config).
