...
**Current engine state:** code merged for the file-an-issue -> daemon-runs-pipeline -> PR-opened -> state-advances loop, but the real workflow contract is not complete. The seeded workflow skills need redesign before dogfooding: rework is incorrectly modeled as a normal sequential stage, deploy is manual-gated in a way that can leave runs non-terminal, `proceed` cannot express a target issue state, and the deploy bridge is terminal-hook-only while review/deploy need PR context earlier. The orchestrator daemon (`npm run daemon`, `ops/systemd/fluxaoaos-daemon.service`) subscribes to `pipeline_run` via Realtime and owns execution end-to-end. The Settings surface exposes Pipelines + Projects tabs. Operators have a Mission Control view at `/[project]/mission-control`. The R-SMOKE happy-path Playwright spec passes against `jdpierke21/fluxaos-alpha-e2e-sandbox`.
