...
**Current engine state:** code merged for the file-an-issue -> daemon-runs-pipeline -> PR-opened -> state-advances loop. The workflow-contract concerns surfaced in the 2026-05-01 FLX-106 audit (rework as sequential stage, manual-gated deploy leaving runs non-terminal, `proceed` lacking a target state, terminal-hook-only deploy bridge) were resolved by the playbook + result-doc model shipped via FLX-106 (workflow redesign), FLX-112 (legacy signal path removed), FLX-129 (personas wired into prompt), and FLX-154 (orchestrator rewrite, persona soul injection) — all merged by 2026-05-06. Spec: [`specs/2026-05-02-pipeline-execution-redesign.md`](specs/2026-05-02-pipeline-execution-redesign.md). The orchestrator daemon (`npm run daemon`, `ops/systemd/fluxaos-daemon.service`) subscribes to `pipeline_run` via Realtime and owns execution end-to-end. The Settings surface exposes Pipelines + Projects tabs. Operators have a Mission Control view at `/[project]/mission-control`. The R-SMOKE happy-path Playwright spec passes against `jdpierce21/fluxaos-alpha-e2e-sandbox`.
