...
| R-RUNTIME — Workspace isolation + forge adapter + deploy bridge | **Done — PR #XX** | [r-runtime-plan](plans/2026-04-23-r-runtime-implementation.md) | [r-runtime-design](specs/2026-04-22-r-runtime-design.md) |

**Current engine state:** The full file-an-issue → get-a-PR loop is wired end-to-end. Every pipeline run executes inside a git worktree isolated under `<repo>/.fluxaos-worktrees/`, on a namespaced `fluxaos/issue-<n>-<run-id>` branch. When a pipeline terminates successfully the orchestrator commits, pushes, opens a PR via the GitHub adapter, records the PR + branch on the issue, and advances the issue to `review`. The cleanup service reaps stale envs; the scheduler refuses to start without operator-configured thresholds. R-REM-W3-a's "engine observed to work" milestone is now "loop observed to close."
...
**Dependency ordering:** R-ARTIFACTS and R-EPIC can run in parallel. R-DAEMON depends on R-RUNTIME (done) and ideally R-ARTIFACTS. R-SETTINGS-ALPHA and R-MISSION-CONTROL are independent of everything. R-SMOKE depends on everything. R-POLISH is last.
