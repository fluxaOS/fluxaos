...
| Phase | Status | Scope |
|-------|--------|-------|
| **R-POLISH — Cleanup, terminology, ship docs** | **Next** | R-SMOKE surfaced two seed-config issues to fix: the seeded `deploy` pipeline_stage has a skill prompt that emits hold/already_complete on every alpha run (R-SMOKE deletes it; the production seed needs a real fix or removal), and review/deploy gate modes are 'hold' (the production seed needs to make sense for the alpha workflow). Terminology passes. README updates so an operator can stand fluxaOS up from a fresh clone. Attribution to Archon in any spec that lifted patterns. Replaces the old R-REM-W4 and R6 placeholders. Also pick up the deferred schema-meta rehydration (drizzle `meta/` snapshots have drifted since migration 0003; R-RUNTIME migration 0007 was hand-written as a result — rebaseline the snapshots so auto-generate is usable again). |

**Dependency ordering:** R-DAEMON + R-SETTINGS-ALPHA + R-MISSION-CONTROL + R-SMOKE done. R-POLISH is last.
