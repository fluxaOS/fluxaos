...
| Phase | Status | Scope |
|-------|--------|-------|
| **R-MISSION-CONTROL — Operator dashboard** | **Next** | One page reading existing orchestrator state: queue depth, in-flight runs, recent terminal states, PR links. No new backend — just a UI over DB state the daemon already writes. |
...
**Dependency ordering:** R-DAEMON + R-SETTINGS-ALPHA done. R-MISSION-CONTROL next (independent of everything). R-SMOKE depends on everything. R-POLISH is last.
