...
| R-REM-W2 — Architecture remediation (realtime registry, stdout-parser port, transactional soft-delete) | **Planned — awaiting execution** | [w2-plan](superpowers/plans/2026-04-18-r-rem-w2-implementation.md) | [w2-design](superpowers/specs/2026-04-18-r-rem-w2-design.md) |
...
5. **R-REM-W2** — Architecture remediation — **Planned, awaiting execution (2026-04-18).** Spec revised after ground-truth reconciliation: three original items dropped (optimistic concurrency on `issue`/`issueComment` already hand-rolled; `pipelineRun`/`stageRun` have a single writer; `src/lib/supabase/` is Next.js SSR glue, not an invariant-7 violation). Revised W2 ships three items: realtime adapter + registry routing (2 consumers migrated), StdoutParser port + subprocess adapter (relocated from `core/orchestrator/output-parser.ts`), transactional `issueCommentService.softDelete`. Plus a doc clarification on invariant 7's scope. Branch `feat/r-rem-w2` carries the plan (9 tasks, TDD shape). Next session executes via `superpowers:subagent-driven-development`. See [w2-design](superpowers/specs/2026-04-18-r-rem-w2-design.md), [w2-plan](superpowers/plans/2026-04-18-r-rem-w2-implementation.md), [planning handoff](superpowers/handoffs/2026-04-18-r-rem-w2-planning-session-handoff.md).
