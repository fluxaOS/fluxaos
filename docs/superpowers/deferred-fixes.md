...
## DEF-021 [RESOLVED 2026-04-24] — R-ARTIFACTS chain journey: stage-2 "Run Stage" click blocked by stage-1 RunDetailModal overlay
...
**Resolution (2026-04-24):** Two independent bugs fixed.

1. `RunDetailModal` had `aria-modal="true"` but no Escape key handler — the aria contract was unimplemented. Added a `window.addEventListener('keydown', ...)` effect that calls `onClose()` on Escape when the modal is open.
2. The chain journey now waits for the backdrop `div[role="dialog"][aria-label="Run detail"]` to detach after pressing Escape, so the next stage's click can't race against a still-mounted modal.
3. Also surfaced a second race: after `stateSelect.selectOption({ label: 'Implement' })`, the Run Stage button reads `matchingStage` from a closed-over render whose `issue.state.key` is still `research` until the tRPC state-transition mutation round-trips. Added a DB poll that waits until `issue_state.key === 'implement'` before clicking, so the click doesn't accidentally re-trigger a research run.

**Follow-up (DEF-022):** Re-running the journey after these fixes surfaced a deeper architectural gap in R-ARTIFACTS: per-click "Run Stage" mints a fresh `pipeline_run` with its own empty artifacts dir, so stage 2 never sees stage 1's `research-findings.md`. Filed as DEF-022 — the stage-to-stage handoff was never actually exercised before.

## DEF-022 [RESOLVED 2026-04-24] — R-ARTIFACTS: artifacts dir not carried across per-stage pipeline_runs on the same issue

**Found:** 2026-04-24 while validating the DEF-021 fix against the live sandbox. Stage 1 + stage 2 both completed cleanly; `plan.md` / PR assertions passed; `research-findings.md` assertion failed because stage 2's `artifacts_path` was a different (empty) directory.
**Severity:** Medium — R-ARTIFACTS's stated purpose (stage-to-stage data flow) is not achievable through the alpha UI. Stage 1 writes an artifact that no subsequent stage can read. Upstream spec line 240 assumed one `pipeline_run` per issue lifecycle; the actual UI flow creates one `pipeline_run` per "Run Stage" click.
**Symptom:** In the DB after the journey runs both stages:
```
pipeline_run[0] artifacts_path = /.../.fluxaos-artifacts/<runId-1>/  ← contains research-findings.md
pipeline_run[1] artifacts_path = /.../.fluxaos-artifacts/<runId-2>/  ← empty
```
The `{{artifacts_path}}` template threaded into stage 2's prompt points at run 2's empty dir. Even if stage 2's skill prompt asks it to read `research-findings.md`, the file isn't there.
**Why it happened:** R-ARTIFACTS's design doc (2026-04-23) assumed "rework creates a new stage_run under the existing pipeline_run, not a new pipeline_run, so artifacts persist." True for rework; false for the per-stage "Run Stage" click, which explicitly calls `svc.createRun(pipelineId, issueId)` in `pipeline.runs.trigger`. No prior handoff mechanism ties sequential pipeline_runs together for the same (pipeline, issue).
**Fix sketch:** On pipeline-run creation (in `pipelineRunService.createRun` or where `artifacts_path` is first computed), look up the most-recent prior terminal pipeline_run for the same `(pipeline_id, issue_id)`. If found and its `artifacts_path` is populated, inherit that path onto the new run instead of deriving a fresh one. The isolation provider then sees an `artifactsPath` param and preserves it (existing logic handles the "already set" case). This matches user intent (stage 2 reads stage 1's findings without a copy step) and keeps per-run row identity.
**Alternative considered:** Adding a `copyForward(priorRunId, newRunId)` FS step. Rejected — extra I/O, extra failure mode, no semantic gain since the directory can safely be shared until the pipeline "closes" (issue reaches `complete`).
**Where:** `src/core/services/pipelineRunService.ts` (`createRun`); `src/server/routers/pipeline.ts:130` (`runs.trigger`). Integration test: add to `src/__tests__/integration/artifacts-cleanup.test.ts` or a new `artifacts-carry-forward.test.ts`. Live journey proof: `e2e/r-artifacts-chain.spec.ts` should pass end-to-end after this change.

**Resolution (2026-04-24):** Threaded through `acquireIsolationEnv` (not `pipelineRunService.createRun`) since the isolation provider is where `artifactsPath` gets resolved and the provider already had a `preserve existing` code path. Added `pipelineId` + `issueId` to `AcquireEnvInput` and a new `findInheritedArtifactsPath` helper that selects the most-recent prior pipeline_run on the same `(pipeline_id, issue_id)` with a populated `artifacts_path`. Passed into `isolation.acquire` via the existing `artifactsPath?` param, which the provider's fresh-mint code path now treats as the preferred path instead of calling `getArtifactsPath(runId)`. Regression test: `src/__tests__/integration/artifacts-inheritance.test.ts` — 3 assertions (inherit on second run, mint fresh on first run, null issueId does not inherit). **Chain journey blocked by DEF-023** (sandbox is empty, Claude emits hold/already_complete regardless). Engine fix proven via integration tests.

## DEF-023 — E2E sandbox is too empty for research stage to exercise chain journey

**Found:** 2026-04-24 while re-running `e2e/r-artifacts-chain.spec.ts` after the DEF-022 fix. Research stage succeeded (wrote `research-findings.md` to its artifacts dir) but emitted `flux:signal = {verdict: 'hold', reason: 'already_complete', meta: {targetState: 'complete'}}`. The `skill_metadata.summary` claimed `src/app/api/health/route.ts already implements all acceptance criteria`, but the sandbox `jdpierce21/fluxaos-alpha-e2e-sandbox` is a bare repo with only an initial commit — no `src/`, no Next.js scaffold, no file at that path. Claude hallucinated the endpoint's presence to justify emitting a terminal signal. Issue advanced straight to `complete`, skipping `implement`. Chain journey never exercises stage 2.
**Severity:** Medium — blocks the R-ARTIFACTS chain journey from being green end-to-end, even though the engine mechanism is correct.
**Proof this is not an engine bug:** DEF-022 integration tests pass (inheritance works at the service layer). R-RUNTIME journey ran stage 1 against the same sandbox on 2026-04-23 and opened a PR successfully — because that journey's assertion is about the deploy-bridge terminal state, not about the research skill emitting `proceed`.
**Why it happens:** The research skill's prompt says *"Read both files. Assess the issue. Do the work described in CLAUDE.md for this issue."* When an agent assessing an empty repo is asked "does this endpoint exist?", the cheapest path for a generative model is to assume yes and emit hold/already_complete. Without a concrete counterexample in the filesystem (a Next.js skeleton that visibly lacks the endpoint), the signal is non-deterministic.
**Fix sketch:** Populate `jdpierce21/fluxaos-alpha-e2e-sandbox` with a minimal Next.js starter (package.json, tsconfig, `src/app/page.tsx` — enough for `src/app/api/health/route.ts` to be a realistic-looking "add this file" ask). Alternative: write a new disposable issue whose task references a file the sandbox actually contains (e.g., "add a comment to `README.md`"). Both approaches tie the journey to concrete filesystem state the skill can observe.
**Where:** Sandbox repo is external (GitHub). Journey: `e2e/r-artifacts-chain.spec.ts`. Seed issue: `src/scripts/db/seed.ts:540-568` (issue #1).

**Resolution (2026-04-24):** Populated `jdpierce21/fluxaos-alpha-e2e-sandbox` with a minimal Next.js 15 App Router skeleton (`package.json`, `tsconfig.json`, `next.config.ts`, `src/app/layout.tsx`, `src/app/page.tsx`, README). No `/api` routes — issue #1's "add /api/health" ask is a real gap Claude can observe. Also committed the fluxaos-managed `.gitignore` entries so future clones don't have to let the isolation provider backfill them. Sandbox now at commit `91e38e9` on `main`.

## DEF-024 [RESOLVED 2026-04-24] — `{{artifacts_path}}` placeholder not substituted in skill prompts materialized to CLAUDE.md

**Found:** 2026-04-24 during DEF-022 / chain-journey validation. After DEF-022 (inheritance) and DEF-023 (sandbox skeleton) fixes, Claude still wrote `research-findings.md` into the worktree root instead of the artifacts dir. Inspection of the materialized `CLAUDE.md` showed the literal string `Write your findings to {{artifacts_path}}/research-findings.md` — the template variable was never substituted. Claude, seeing a literal path that looked broken, wrote the file to `./research-findings.md` in its cwd (the worktree).
**Severity:** High — the R-ARTIFACTS mechanism silently worked the wrong way for every stage since W5 shipped. Engine wrote `artifacts_path` to DB rows, but AI agents couldn't actually use the directory because the prompt told them to use a literal placeholder. Stage-to-stage handoff via artifacts was architecturally sound but functionally dead.
**Symptom:** Materialized `CLAUDE.md` contained `{{artifacts_path}}/research-findings.md` literally. Claude wrote findings to `<worktree>/research-findings.md`. Downstream stages can't find the file at the expected path. Test's assertion-against-`{{artifacts_path}}/research-findings.md` fails: file missing, because it's actually at a different location.
**Why it happened:** `stage-runner.ts:248` runs `renderTemplate(driverRow.issuePromptTemplate, { artifacts_path: ... })` on the **driver prompt** that's passed to the binary (`claude --prompt ...`). But the skill's `promptTemplate` flows through `materialize()` → directly written into `CLAUDE.md` at `src/core/skills/materializer.ts:87` with no substitution. `ARTIFACTS_SUFFIX` in seed.ts put `{{artifacts_path}}` in the skill prompt, assuming it'd be rendered somewhere — it wasn't.
