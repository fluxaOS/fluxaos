...
No migrations. No schema changes. No adapter changes outside the port type. The payload-shape change is forward-compatible: old-shape events (the previous `{...entry, content}` superset) are still valid `TranscriptEntry` records because the `...entry` spread already included every typed field; the extra `content` is ignored by the new consumer.
...
| Risk | Mitigation |
|---|---|
| Some other consumer reads `payload.content` on output events | Audited — only LiveOutput (being rewritten) and `src/scripts/db/events.ts` (dumps `JSON.stringify(payload)` wholesale, no field-pick). |
