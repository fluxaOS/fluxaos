...
> **STATUS: SUPERSEDED.** This was the implementation plan for the FLX-106 playbook/YAML experiment that shipped in PR #203 and was later replaced by DB-owned, config-driven stage routing. Treat the detailed tasks below as historical implementation record only; do not use them as active operator or architecture guidance. Current truth: pipeline stages, skills, drivers, gates, routing, and deploy behavior are database configuration seeded by `npm run db:seed`, and the active daemon executes that DB graph (`research` → `implement` → `review` → `rework` → `deploy` in the seeded alpha lifecycle). PR #313 (`flx-197-pipeline-truth`) removes the remaining code-side playbook path.
>
> Original completion note: all four phases shipped on branch `flx-106-pipeline-execution-redesign` (PR #203). Plan-review issues C1/C4/C6/I4/I6/I8 resolved in implementation. Post-implementation review fixes (undefined `log`, stage_run lifecycle, blocked routing, LangGraph checkpointer wiring) committed 2026-05-03 in `915ee6d`. Checkboxes below were never ticked — work was completed before the design was superseded. Follow-up issues logged at the time: FLX-107 (parallel groups), FLX-108 (typed `complete` constant), and two new issues for Archon-style node variants and triage-as-meta-stage. See memory: `project_flx106_architecture_decision.md`.

> **Historical worker note:** The task checklist below is preserved for auditability only. Do not execute it; use the current DB-driven pipeline docs instead.
