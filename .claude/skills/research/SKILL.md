...
{{PARTIAL:parallel-agent-spawn}}

**Agent prompt template** (fill in per issue):
```
Research and plan issue #<NUMBER> for the {{PROJECT}} project.
...
## fluxaOS Signal Emission (MANDATORY)

**When running inside a fluxaOS pipeline stage, you MUST emit a signal as your very last action.** Use the Bash tool to echo the signal JSON. This is how the orchestrator knows your outcome.

**On success** (research complete, plan posted, state transitioned to implement):
```bash
echo '{"flux:signal": {"verdict": "proceed", "summary": "<one-line summary of what was researched/planned>"}}'
```

**On hold** (blocked, needs external input, cannot proceed):
```bash
echo '{"flux:signal": {"verdict": "hold", "summary": "<what is blocking progress>"}}'
```

**On failure** (cannot complete research, returned to on-hold):
```bash
echo '{"flux:signal": {"verdict": "abort", "summary": "<why research failed>"}}'
```

**Rules:**
- The signal MUST be the very last Bash command you execute
- Emit exactly one signal — if you emit multiple, only the last one counts
- The `summary` field should be a concise one-liner describing the outcome
- If you are NOT running inside a fluxaOS pipeline (e.g., invoked directly by a user), skip this step
