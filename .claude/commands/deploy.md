...
- If complex conflicts: set state to `rework` (`flu issue state <number> rework`), comment with details, move to next issue
...
```bash
flu issue comment <number> --body "## Merge Blocked -- Tests Fail After Rebase
...
# Create PR and merge
git checkout main
flu pr create --title "Description (#<issue>)" --head <branch>
flu pr merge <pr-number>
...
**If `false` is `false` AND `` is empty, skip the service restart and log check -- no service to check.**

If `false` is `true`:
```bash
flu service webapp restart
```

#### 4a. Post-Restart Log Check -- MANDATORY (NO EXCEPTIONS, BLOCKING)

**You MUST check service logs for startup errors before proceeding. This step is NOT conditional -- execute it every time a service exists. Skipping is NOT permitted when `` is not empty.**
...
sudo journalctl -u  --since "2 minutes ago" --no-pager 2>/dev/null | grep -iE "error|exception|traceback|critical|fatal" | head -20
```
...
| Service failed to start (exit code != 0) | **FAIL** | STOP -- check `systemctl status ` |
...
**If `false` is `false`, skip this step -- no application log file for this project.**

If `false` is `true`:

**You MUST check the application's own log file for errors that don't surface in journalctl. Skipping is NOT permitted when `false` is `true`.**
...
```bash
flu issue comment <number> --body "### Post-Merge Verification
...
#### 4c. Deployed Webapp Verification (webapp=false)

**If `false` is `false`, skip this step -- this project has no webapp.**

If `false` is `true`, you MUST visit the deployed URL in a real browser and verify the feature works.
...
flu issue comment <number> --body "Completed via PR #X (vX.X.X)"
flu issue close <number>
flu issue state <number> completed
```
...
git push --force-with-lease origin <branch>
git checkout main
flu pr create --title "Title (#num)" --head <branch>
flu pr merge <pr>
git pull origin main

# Test Deployment (only if false is true)
flu service webapp restart
...
# Close and update state
flu issue comment <num> --body "Completed via PR #X (vX.X.X)"
flu issue close <num>
flu issue state <num> completed
```
