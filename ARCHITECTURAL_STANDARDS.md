<!-- DO NOT EDIT this file directly. It is maintained by fhc sync and will be overwritten.
     Source: templates/root-files/ARCHITECTURAL_STANDARDS.md in fh-commons -->
...
11. Homelab acceptance requires real production workflow verification: smallest real check first, then the full affected workflow.
...
def get_config():
    if not config_file.exists():
        raise ConfigNotFoundError(
            ...
            f"Run ' install' to create configuration."
        )
    return load_config()
```
...
Run ' install' to create configuration.
```
...
- **API calls:** `src/fluxaos/api/platform_client.py`
...
### Homelab Acceptance Verification

This project runs in a one-user homelab where the real environment is the
acceptance environment. Passing automated tests is not enough when the changed
behavior is an operational workflow.

For every non-trivial change:

1. Run the smallest real command that proves the changed behavior.
2. Run the full real workflow for the affected surface.
3. Inspect downstream evidence, not just command success text.
4. Report any warnings as follow-up work.

For ` sync` changes specifically, acceptance means:
...
Synthetic E2E tests with `tmp_path`, local git repos, local bare origins, or
HTTP stubs are regression coverage only. They do not replace live homelab
acceptance unless the user explicitly forbids production verification.
...
|----------|-------------|-----------|---------|
| Project | `cli.json` → `commands` | `_execute_project_command()` | ` run`, ` status` |
| Shared | `cli.json` → `shared_commands` | `_execute_shared_command()` | ` version`, ` doctor` |
| Delegated | `registry.py` → `DELEGATED_COMMANDS` | `_delegate_to_fhc()` | ` git`, ` issue` |
...
### Memory `category` is legacy (spec §9.4)

**Rule:** New code MUST NOT read or write `memory_entries.category`.
The canonical class column is `memory_class`.

**Enforced by:** `tests/e2e/test_memory_no_category_in_new_code.py` greps
`src/fluxaos/` for `.category`, `category=`, and `"category":` and
fails on any match outside an explicit allowlist. Legacy modules in the
allowlist — the openclaw-bridge ingest endpoint, the ORM model
declaration, and the backfill mapping table — exist BY DESIGN and are
the only approved sites.

**If your code fails this check:** either refactor the new code to use
`memory_class`, or — if the new code is a genuine extension of the
legacy surface — propose adding it to the allowlist in a PR and update
this rule.
