...
function assertContains(relativePath: string, haystack: string, needle: string): void {
...
function assertNearby(
  ...
  for (let index = haystack.indexOf(anchor); index !== -1; index = haystack.indexOf(anchor, index + 1)) {
    foundAnchor = true;
...
for (const spec of deploySpecs) {
  ...
  assertNearby(spec, content, 'FLUXAOS_TARGET_REPO_PATH', 'project.target_repo_path');
