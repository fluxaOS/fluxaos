-- FLX-222 (2026-05-11): migrate FLUXAOS_WORKSPACE_ROOT from env to
-- config_entry (scope=`'global'`, project_id=NULL, key=`runtime.workspace_root`).
--
-- The env path is being removed; the DB is the new sole source of truth
-- (no fallbacks). We insert the row with value=jsonb null, which is the
-- explicit "use the adapter's built-in in-project layout" choice. An
-- operator who previously set FLUXAOS_WORKSPACE_ROOT in .env should update
-- this row to the JSON-encoded absolute path string after migration, via
-- Settings → System or:
--
--   UPDATE config_entry
--   SET value = to_jsonb('/srv/flux/workspaces'::text),
--       version = version + 1,
--       previous_value = value,
--       updated_at = now()
--   WHERE scope = 'global' AND project_id IS NULL
--     AND key = 'runtime.workspace_root';
--
...
SELECT 'global', NULL, 'runtime.workspace_root', 'null'::jsonb, 1
WHERE NOT EXISTS (
