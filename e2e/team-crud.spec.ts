...
//
// FLX-239 Stage 7 (FLX-266): rewritten for the org-scoped team model.
// Teams now belong to an organization (team.org_id); the Settings → Teams
// page resolves the project from the /p/{uuid} URL and team.create derives
// the org from that project. The CRUD round-trip below exercises exactly
// that path. Stray rows from failed runs are swept by deterministic name
// prefix (see helpers/catalog-cleanup.ts) so seed verification stays green.

import { cleanupTeamCrudRows } from './helpers/catalog-cleanup';
import { expect, projectPath, test } from './helpers/setup';

test.describe('@flx-61 @journey @team-crud', () => {
  test.beforeAll(async () => {
    await cleanupTeamCrudRows();
  });
  ...
  test('Teams tab renders + Create / Edit / Delete round-trip', async ({
    ...
    // The new team has no team_member rows, so the router's member-count
    // guard allows the delete.
