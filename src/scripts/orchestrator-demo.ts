...
async function demo() {
  ...
  for (const s of stages) {
    log(
      '  ',
      `${s.sortOrder}. ${s.name} — gate: ${s.gateMode}, driver: ${s.driver}`
    );
  }
  ...
  const [proj] = await db
    .select()
    .from(project)
    .where(eq(project.id, pipe.projectId));
  const providers = await db
    .select()
    .from(provider)
    .where(eq(provider.orgId, proj.orgId));
  ...
  let [iss] = await db
    .select()
    .from(issue)
    .where(eq(issue.projectId, pipe.projectId))
    .limit(1);
  if (!iss) {
    const [type] = await db
      .select()
      .from(issueType)
      .where(eq(issueType.projectId, pipe.projectId))
      .limit(1);
    const [state] = await db
      .select()
      .from(issueState)
      .where(eq(issueState.projectId, pipe.projectId))
      .limit(1);
    const [status] = await db
      .select()
      .from(issueStatus)
      .where(eq(issueStatus.projectId, pipe.projectId))
      .limit(1);
    const [priority] = await db
      .select()
      .from(issuePriority)
      .where(eq(issuePriority.projectId, pipe.projectId))
      .limit(1);
  ...
  log(
    '📊',
    `Pipeline run: ${finalRun?.status} (cost: $${finalRun?.totalCostUsd})`
  );
  ...
  for (const sr of finalStageRuns) {
    ...
    log(
      '  ',
      `${stageDef?.name ?? '?'}: ${sr.status} (${events.length} events, provider: ${sr.provider ?? 'n/a'}, model: ${sr.model ?? 'n/a'})`
    );
  }
  ...
  for (const sr of finalStageRuns) {
    ...
    await db
      .delete(stageGateResult)
      .where(eq(stageGateResult.stageRunId, sr.id))
      .catch(() => {});
    await db
      .delete(stageRun)
