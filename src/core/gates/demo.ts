...
function printResult(label: string, result: any) {
  ...
  if (result.ruleResults.length > 0) {
    ...
    for (const rr of result.ruleResults) {
      ...
      console.log(`   ${ri} ${rr.rule.field} ${rr.rule.operator} ${JSON.stringify(rr.rule.value)}${label}`);
      if (!rr.passed) {
        console.log(`      → actual: ${JSON.stringify(rr.actualValue)} — ${rr.reason}`);
      }
    }
  }
}
...
async function demo() {
  ...
  for (const s of stages) {
    const hasRules = s.gateRules && typeof s.gateRules === 'object' && 'logic' in (s.gateRules as any);
    console.log(`  ${s.sortOrder}. ${s.name} — gate mode: ${s.gateMode}${hasRules ? ' (rules configured)' : ''}`);
  }

  // ── 2. Pure engine: test gate modes ──────────────────────────────────
  console.log('\n── Gate Mode Tests (pure engine, no DB) ───────────\n');

  printResult('auto mode — always proceeds',
    evaluateGate('auto', null, {}));

  printResult('hold mode — always holds',
    evaluateGate('hold', null, {}));

  printResult('skip mode — always proceeds',
    evaluateGate('skip', null, {}));

  // ── 3. Pure engine: rule evaluation ──────────────────────────────────
  console.log('\n── Rule Evaluation Tests ───────────────────────────\n');

  const rulesStage = stages.find((s) => s.gateMode === 'rules');
  if (rulesStage && rulesStage.gateRules) {
    const rules = rulesStage.gateRules as RuleGroup;
    const mode = rulesStage.gateMode as GateMode;

    printResult(`${rulesStage.name}: exit_code=0, cost_usd=5 (should PASS)`,
      evaluateGate(mode, rules, { exit_code: 0, cost_usd: 5 }));

    printResult(`${rulesStage.name}: exit_code=1, cost_usd=5 (should REWORK — bad exit)`,
      evaluateGate(mode, rules, { exit_code: 1, cost_usd: 5 }));

    printResult(`${rulesStage.name}: exit_code=0, cost_usd=50 (should HOLD — over budget)`,
      evaluateGate(mode, rules, { exit_code: 0, cost_usd: 50 }));

    printResult(`${rulesStage.name}: exit_code=1, cost_usd=50 (should REWORK — worst wins)`,
  ...
  const orRules: RuleGroup = {
    logic: 'AND',
    rules: [
      { field: 'exit_code', operator: 'equals', value: 0, severity: 'required', onFail: 'hold', label: 'Clean exit' },
      {
        logic: 'OR',
        rules: [
          { field: 'env', operator: 'equals', value: 'staging', severity: 'required', onFail: 'hold', label: 'Staging env' },
          { field: 'env', operator: 'equals', value: 'production', severity: 'required', onFail: 'hold', label: 'Production env' },
        ],
      },
    ],
  };

  printResult('OR group: exit=0, env=staging (should PASS)',
    evaluateGate('rules', orRules, { exit_code: 0, env: 'staging' }));

  printResult('OR group: exit=0, env=dev (should HOLD — wrong env)',
    evaluateGate('rules', orRules, { exit_code: 0, env: 'dev' }));

  const nestedRules: RuleGroup = {
    logic: 'AND',
    rules: [
      { field: 'output.tests.passed', operator: 'equals', value: true, severity: 'required', onFail: 'rework', label: 'Tests passed' },
      { field: 'output.version', operator: 'matches', value: '^v\\d+\\.\\d+', severity: 'required', onFail: 'hold', label: 'Version format' },
      { field: 'tags', operator: 'contains', value: 'reviewed', severity: 'warn', onFail: 'notify', label: 'Has review tag' },
    ],
  };

  printResult('Nested context + regex: all pass',
    evaluateGate('rules', nestedRules, {
      output: { tests: { passed: true }, version: 'v2.3' },
      tags: ['tested', 'reviewed'],
    }));

  printResult('Nested context: tests failed (should REWORK)',
    evaluateGate('rules', nestedRules, {
      output: { tests: { passed: false }, version: 'v2.3' },
      tags: ['tested', 'reviewed'],
    }));

  printResult('Warn severity: missing review tag (should still PROCEED)',
    evaluateGate('rules', nestedRules, {
      output: { tests: { passed: true }, version: 'v2.3' },
      tags: ['tested'],
    }));
  ...
  if (rulesStage) {
    ...
    printResult(`Service: evaluated ${rulesStage.name} from DB (exit=0, cost=3.50)`, r);
    ...
    console.log(`   📋 ${audits.length} audit record(s) found for this stage run`);
    if (audits[0]) {
      console.log(`      verdict: ${audits[0].verdict}, passed: ${audits[0].passed}`);
    }
